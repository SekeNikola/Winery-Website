...
class CI_Ads125 extends WP_Widget {
	...
	function __construct(){
		...
		parent::__construct( 'ci_ads125_widget', $name = __( '-= CI 125x125 Ads =-', 'truenorth' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		echo '<p><label for="' . esc_attr( $this->get_field_id( 'ci_title' ) ) . '">' . __( 'Title', 'truenorth' ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_title' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_title' ) ) . '" type="text" value="' . esc_attr( $ci_title ) . '" class="widefat" /></p>';
		echo '<p><input id="' . esc_attr( $this->get_field_id( 'ci_random' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_random' ) ) . '" type="checkbox"' . checked( $instance['ci_random'], 'random' ) . ' value="random" /> <label for="' . esc_attr( $this->get_field_id( 'ci_random' ) ) . '">' . __( 'Display ads in random order?', 'truenorth' ) . '</label></p>';
		echo '<p><input id="' . esc_attr( $this->get_field_id( 'ci_new_win' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_new_win' ) ) . '" type="checkbox"' . checked( $instance['ci_new_win'], 'enabled' ) . ' value="enabled" /> <label for="' . esc_attr( $this->get_field_id( 'ci_new_win' ) ) . '">' . __( 'Open ads in new window?', 'truenorth' ) . '</label></p>';
		...
		for ( $i = 1; $i <= 8; $i ++ ) {
			...
			echo '<label for="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'url' ) ) . '">' . sprintf( __( 'Banner #%d URL', 'truenorth' ), $i ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'url' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_b' . $i . 'url' ) ) . '" type="text" value="' . esc_attr( $url ) . '" class="widefat" />';
			echo '<label for="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'lin' ) ) . '">' . sprintf( __( 'Banner #%d Link', 'truenorth' ), $i ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'lin' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_b' . $i . 'lin' ) ) . '" type="text" value="' . esc_attr( $lin ) . '" class="widefat" />';
			echo '<label for="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'tit' ) ) . '">' . sprintf( __( 'Banner #%d Title', 'truenorth' ), $i ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_b' . $i . 'tit' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_b' . $i . 'tit' ) ) . '" type="text" value="' . esc_attr( $tit ) . '" class="widefat" />';
			?></p><?php
		}
