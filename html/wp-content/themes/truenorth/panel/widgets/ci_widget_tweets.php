...
class CI_Tweets extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'ci_twitter_widget', $name = __( '-= CI Tweets =-', 'truenorth' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $data['errors'] ) && count( $data['errors'] ) > 0 ) {
			foreach ( $data['errors'] as $error ) {
				...
				echo '<li>' . sprintf( __( 'Error %1$s: %2$s', 'truenorth' ), $error['code'], $error['message'] ) . '</li>';
			}
		} else {
	...
	function form($instance){
		...
		if ( ci_setting( 'twitter_consumer_key' ) == '' or
			...
			echo '<p>' . __( "It looks like you haven't provided Twitter access details, in order for this widget to work. Unfortunately, this is needed. Please visit the theme's settings page and provide the required access details.", 'truenorth' ) . '</p>';
		} else {
			...
			echo '<p><label for="' . esc_attr( $this->get_field_id( 'ci_username' ) ) . '">' . __( 'Username:', 'truenorth' ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_username' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_username' ) ) . '" type="text" value="' . esc_attr( $ci_username ) . '" class="widefat" /></p>';
			echo '<p><label for="' . esc_attr( $this->get_field_id( 'ci_number' ) ) . '">' . __( 'Number of tweets:', 'truenorth' ) . '</label><input id="' . esc_attr( $this->get_field_id( 'ci_number' ) ) . '" name="' . esc_attr( $this->get_field_name( 'ci_number' ) ) . '" type="text" value="' . esc_attr( $ci_number ) . '" class="widefat" /></p>';
		}
