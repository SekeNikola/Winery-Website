...
function ci_register_theme_default_scripts() {
	...
	wp_register_script( 'retinajs', get_child_or_parent_file_uri( '/panel/components/retinajs/dist/retina.js' ), array(), '2.1.2', true );
	wp_register_style( 'font-awesome', get_child_or_parent_file_uri( '/panel/components/fontawesome/css/font-awesome.min.css' ), array(), '4.7.0' );
...
function ci_settings_page() {
	...
	<div class="wrap">
		<h2>
			<?php
				if ( ! CI_WHITELABEL ) {
					...
					echo sprintf( _x( '%1$s Settings v%2$s', 'theme name settings version', 'truenorth' ),
						TRUENORTH_NICENAME,
						TRUENORTH_VERSION
					);
				} else {
					echo sprintf( _x( '%s Settings', 'theme name settings', 'truenorth' ),
						TRUENORTH_NICENAME
					);
				}
			?>
		...
		<?php if ( ( $latest_version !== false ) && version_compare( $latest_version, TRUENORTH_VERSION, '>' ) ): ?>
			<div class="notice notice-info theme-update">
				<p>
					<?php echo sprintf( __( 'A theme update is available. The latest version is <b>%1$s</b> and you are running <b>%2$s</b>', 'truenorth' ), $latest_version, TRUENORTH_VERSION ); ?>
				</p>
			</div>
		<?php endif; ?>
		...
		<div id="ci_panel" class="<?php echo implode( ' ', $panel_classes ); ?>">
			<form method="post" action="options.php" id="theform" enctype="multipart/form-data">
				...
				<?php if ( isset( $_POST['reset'] ) ): ?>
					<div class="resetbox"><?php _e( 'Settings reset!', 'truenorth' ); ?></div>
				<?php endif; ?>
...
function ci_panel_upload_image( $fieldname, $label, $params = array() ) {
	...
	<input type="submit" class="ci-upload button" value="<?php esc_attr_e( 'Upload image', 'truenorth' ); ?>"/>
	<div class="up-preview"><?php echo( ! empty( $preview_url ) ? '<img src="' . esc_url( $preview_url ) . '" />' : '' ); ?></div>
	<?php if ( ! empty( $preview_url ) ): ?>
		<div class="up-preview-bg group">
			<span class="swatch white" title="<?php echo esc_attr( sprintf( _x( 'Preview in %s background', 'preview in color background', 'truenorth' ), _x( 'White', 'color', 'truenorth' ) ) ); ?>"></span>
			<span class="swatch grey" title="<?php echo esc_attr( sprintf( _x( 'Preview in %s background', 'preview in color background', 'truenorth' ), _x( 'Grey', 'color', 'truenorth' ) ) ); ?>"></span>
			<span class="swatch black" title="<?php echo esc_attr( sprintf( _x( 'Preview in %s background', 'preview in color background', 'truenorth' ), _x( 'Black', 'color', 'truenorth' ) ) ); ?>"></span>
			<span class="swatch transparent" title="<?php echo esc_attr( sprintf( _x( 'Preview in %s background', 'preview in color background', 'truenorth' ), _x( 'Transparent', 'color', 'truenorth' ) ) ); ?>"></span>
		</div>
	<?php endif; ?>
<?php
}
