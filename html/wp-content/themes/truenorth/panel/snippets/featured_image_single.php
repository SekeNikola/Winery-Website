	...
	<fieldset id="ci-panel-featured-image-single" class="set">
		...
		<?php
			...
			foreach($thumb_types as $post_type)
			{
				...
				ci_panel_checkbox('featured_single_'.$post_type.'_show', 'enabled', sprintf(__('Show featured images on <em>%s</em>', 'truenorth'), $obj->labels->name));
			}
		?>
		<fieldset class="mt10">
