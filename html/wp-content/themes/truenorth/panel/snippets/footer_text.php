...
<?php else: ?>
	...
	<?php if( has_filter('ci_footer_credits') ): ?>
		...
		<fieldset id="ci-panel-footer-text" class="set">
			<legend><?php _e('Footer Text', 'truenorth'); ?></legend>
			<?php $allowed_tags = apply_filters('ci_footer_allowed_tags', array('<a>','<b>','<strong>','<i>','<em>','<span>')); ?>
			<p class="guide"><?php echo apply_filters('ci_panel_footer_credits_description', sprintf(__('You can change the footer text by entering your custom text here. You may use <strong>:year:</strong> to display the current year. The following HTML tags are allowed: %s', 'truenorth'), htmlspecialchars(implode(' ', $allowed_tags)) )); ?></p>
