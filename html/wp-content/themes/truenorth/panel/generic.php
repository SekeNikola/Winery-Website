...
{
	if( TRUENORTH_UPDATES === false ) return;
...
function truenorth_update_check_admin_handler($transient)
{
	if(TRUENORTH_UPDATES === true)
	{
		...
		if(($latest_version !== false) and version_compare($latest_version, TRUENORTH_VERSION, '>'))
		{
			$theme_slug = 'wp_'.TRUENORTH_NAME.'5';
			
			$transient->checked[ TRUENORTH_NAME ] = TRUENORTH_VERSION;
			$transient->response[ $theme_slug ] = array(
				'new_version' => $latest_version,
				'url' => 'http://www.cssigniter.com/ignite/themes/'.TRUENORTH_NAME,
				'package' => ''
			);
		}
	}
