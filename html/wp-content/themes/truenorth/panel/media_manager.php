...
function ci_enqueue_media_manager_scripts() {
	...
	if ( ! wp_script_is( 'ci-media-manager-3-5', 'enqueued' ) ) {
		$settings = array(
			...
			'tUseThisFile'        => __( 'Use this file', 'truenorth' ),
			'tUpdateGallery'      => __( 'Update gallery', 'truenorth' ),
			'tLoading'            => __( 'Loading...', 'truenorth' ),
			'tPreviewUnavailable' => __( 'Gallery preview not available.', 'truenorth' ),
			'tRemoveFromGallery'  => __( 'Remove from gallery', 'truenorth' ),
		);
...
function ci_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	...
	<div class="ci-media-manager-gallery">
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php _e( 'Randomize order', 'truenorth' ); ?></label></p>
